...
router.patch('/:id', auth, async (req, res)=>{
    ...
    if (!isUpdateValid) return res.status(400).send({error: 'Property not found.'});
    try{
        ...
        if(likedAlready === -1) { // case like comment
            comment = {...comment, likes: comment.likes+1, likers: [...comment.likers,req.user._id ]}
        } else { // unlike comment
            comment = {...comment, likes: comment.likes-1, likers: comment.likers.splice(likedAlready, 1)};
        }
...
router.delete('/:id', auth, async(req, res)=>{
    try{
        const comment = Comment.deleteOne({_id: req.params.id,  owner: {name: req.user.nickname}})
    } catch(e){
        res.status(500).send({error: e});
    }
});
